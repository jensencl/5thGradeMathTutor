...
def gen_universal_dynamic_story():
    name = random.choice(["Charli", "Annabelle", "Graysen", "Evie"])
...
def gen_mh_u2_stacked_boxes_table():
    ...
    wa = random.choice([3, 4])  # Box A/B depth (shared)
    wb = wa  # Shared depth

    # Box B length is wider than Box A
    lb = la + random.choice([2, 3])

    # Shared height for both boxes, keeping numbers easy (e.g., 3 to 6)
    h_shared = random.randint(3, 6)
    ...
    return {
        ...
        "lesson": "Lesson 2-4",
        "hint": (
            f"Box B's base area is ({lb} × {wb}) = {lb * wb} sq in., and Box A's base area "
            f"is ({la} × {wa}) = {la * wa} sq in. Since they share the same height ({h_shared} in.), "
            f"divide the total volume ({tot_vol}) by the total base area to find the height!"
        ),
        ...
        "blank_fields": [
            {
                ...
                "placeholder": "Enter height in inches (e.g., 3)",
            },
            {
                "key": "va",
